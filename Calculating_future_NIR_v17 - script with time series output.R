...
library("readxl")
codes=read_excel(
  "C:/Users/edigm/Documents/Joint MSc/Thesis Work/CMIP5_climate_data_monthly/NUTS3_codes/County level Yield of 10 field Crops_2016.xlsx",
...
##read in shape file 
####### 
NUTS3_shp=st_read(dsn="C:/Users/edigm/Documents/Joint MSc/Thesis Work/CMIP5_climate_data_monthly/NUTS3_shapefile/VG250_KRS.shp")
  ...
  function_RCP <- function(RCP){
    ...
    function_meteo <- function(meteo){
      ...
      climate_monthly_input <- read_csv(paste(
        "C:/Users/edigm/Documents/Joint MSc/Thesis Work/CMIP5_climate_data_monthly/climate_monthly_input_NorESM1-M_" 
...
Elevation <- read.csv ("C:\\Users\\edigm\\Documents\\Joint MSc\\Thesis Work\\CMIP5_climate_data_monthly\\DE_NUTS3_elev_3_edited_fixedver.csv", header = TRUE)
...
#we add another list for time series data, "TS_default"
  TS_default <- list()
...
#add the test01 field to the time series data list:
TS_default[[length(TS_default) + 1]] <- test01
...
#we do the same for TS_default:
TS_default <-  do.call ( rbind.data.frame, TS_default) 
assign ( (paste0 ("TS_", cropdat$cropcode[c], "_", cropdat$cropname[c] ) ), 
         TS_default )
...
#####################added: averaging for time series sets:#####################
#make a big list:
TS_data <- list (TS_1_drybeans, TS_2_onions, TS_3_cabbage, TS_4_asparagus, TS_5_alfalfa, TS_6_apples, TS_7_cherries)
TS_data <-  do.call ( rbind.data.frame, TS_data)

#calculate the federal-level average per crop:
#splitting big dataframe into three via RCPs
TS_list <- split (TS_data, TS_data$rcp)

#averaging state-level data for each RCP
avgTS_2p6 <- TS_list[["rcp2p6"]]%>%
  group_by(cropcode, cropname, year) %>%
  summarise(P_2p6 = mean (P, na.rm = TRUE), #average precipitation
            ETo_2p6 = mean (ETo_PM, na.rm = TRUE), #Ref. ET
            ETc_2p6 = mean (ETc_adj, na.rm = TRUE), #average crop ET
            Y_2p6 = mean (Y_a, na.rm = TRUE) #average yield
  )%>%
  ungroup()

avgTS_6p0 <- TS_list[["rcp6p0"]]%>%
  group_by(cropcode, cropname, year) %>%
  summarise(P_6p0 = mean (P, na.rm = TRUE), #average precipitation
            ETo_6p0 = mean (ETo_PM, na.rm = TRUE), #Ref. ET
            ETc_6p0 = mean (ETc_adj, na.rm = TRUE), #average crop ET
            Y_6p0 = mean (Y_a, na.rm = TRUE) #average yield
  )%>%
  ungroup()

avgTS_8p5 <- TS_list[["rcp8p5"]]%>%
  group_by(cropcode, cropname, year) %>%
  summarise(P_8p5 = mean (P, na.rm = TRUE), #average precipitation
            ETo_8p5 = mean (ETo_PM, na.rm = TRUE), #Ref. ET
            ETc_8p5 = mean (ETc_adj, na.rm = TRUE), #average crop ET
            Y_8p5 = mean (Y_a, na.rm = TRUE) #average yield
  )%>%
  ungroup()

#merging all state-level averages into one table, different columns per RCP
avgTS <- merge ( merge (avgTS_2p6, avgTS_6p0, by = c("year", "cropcode", "cropname" )), avgTS_8p5, by = c("year", "cropcode", "cropname" ) )

#removing all extra datasets.
remove(avgTS_2p6, avgTS_6p0, avgTS_8p5, TS_list)


#add function to add midpoint year group
create_year_group <- function(year) {
  midpoint_year <- floor((year - 1990) / 5) * 5 + 1990 + 2  # Calculate midpoint year
  as.integer(midpoint_year)
}


# Group data by 5-year intervals and compute average
#generate 5-year averages to smooth out plots:
  avgTS_5 <- avgTS %>%
  mutate(year_group_midpoint = create_year_group(year)) %>%
  group_by(cropcode, cropname, year_group_midpoint) %>%
  summarise(P_2p6 = mean (P_2p6, na.rm = TRUE), #average precipitation
            ETo_2p6 = mean (ETo_2p6, na.rm = TRUE), #Ref. ET
            ETc_2p6 = mean (ETc_2p6, na.rm = TRUE), #average crop ET
            Y_2p6 = mean (Y_2p6, na.rm = TRUE), #average yield
            
            P_6p0 = mean (P_6p0, na.rm = TRUE), #average precipitation
            ETo_6p0 = mean (ETo_6p0, na.rm = TRUE), #Ref. ET
            ETc_6p0 = mean (ETc_6p0, na.rm = TRUE), #average crop ET
            Y_6p0 = mean (Y_6p0, na.rm = TRUE), #average yield
            
            P_8p5 = mean (P_8p5, na.rm = TRUE), #average precipitation
            ETo_8p5 = mean (ETo_8p5, na.rm = TRUE), #Ref. ET
            ETc_8p5 = mean (ETc_8p5, na.rm = TRUE), #average crop ET
            Y_8p5 = mean (Y_8p5, na.rm = TRUE) #average yield
...
#split big list by crop:
avgTS_crops <- split (avgTS, avgTS$cropname)

#split averaged list by crop:
avgTS_5_crops <- split (avgTS_5, avgTS_5$cropname)


#generate a plot!
ggplot() + 
  geom_line ( data = avgTS_crops[["alfalfa"]], aes (x = year, y = ETc_2p6, color = "Dataset 1") ) +
  geom_line ( data = avgTS_crops[["alfalfa"]], aes (x = year, y = ETc_6p0, color = "Dataset 2") ) +
  geom_line ( data = avgTS_crops[["alfalfa"]], aes (x = year, y = ETc_8p5, color = "Dataset 3") ) +
  labs (x = "year", y = "Evapotranspiration (mm / year)", title = "testgraph") +
  scale_color_manual (values = c("green", "orange", "red"))
  
#generate an averaged plot!
ggplot() + 
  geom_line ( data = avgTS_5_crops[["alfalfa"]], aes (x = year_group_midpoint, y = ETc_2p6, color = "RCP 2.6") ) +
  geom_line ( data = avgTS_5_crops[["alfalfa"]], aes (x = year_group_midpoint, y = ETc_6p0, color = "RCP 6.0") ) +
  geom_line ( data = avgTS_5_crops[["alfalfa"]], aes (x = year_group_midpoint, y = ETc_8p5, color = "RCP 8.5") ) +
  geom_smooth(method = "loess", color = "red", se = FALSE) +  # Smoothed line
  labs (x = "year", y = "Annual Evapotranspiration (mm)", title = "Crop Evapotranspiration (5-year average)") +
  scale_color_manual (values = c("green", "orange", "red"))

#above works!

##ANOTHER STEP##
#we have the raw climate data, "THR_set"
#this is done by year & by month (01-12)
#we need to average this data to get the mean temps + precipitation monthly
#then we need to average it more to get the yearly values for spring/summer (march - august) and autumn/winter (sept - feb)

#generate the weather data from THR_set, demarcate spring/summer (march - august) and autumn/winter (sept-feb) weather
THR_avg_weather <- THR_set %>%
  mutate(season = ifelse( THR_set$month %in% c ("03", "04", "05", "06", "07", "08"), "spring/summer", "autumn/winter")
          )%>%
  group_by(year, rcp, season) %>%
  summarise(P = sum (P, na.rm = TRUE), #average precipitation
            Tmean = mean (Tmean, na.rm = TRUE), #mean temp
            
  )

#split the summer and winter sets, and re-merge so they're sharing columns
THR_avg_summer <- subset (THR_avg_weather, THR_avg_weather$season == "spring/summer", select = -season)
THR_avg_winter <- subset (THR_avg_weather, THR_avg_weather$season == "autumn/winter", select = -season)
THR_avg_weather <- merge (THR_avg_summer, THR_avg_winter, by = c( "year", "rcp"), suffixes = c("_summer", "_winter"))

#remove excess dataframes:
remove (THR_avg_summer, THR_avg_winter)

#calculate 5-year averages:

THR_avg_weather <- THR_avg_weather %>%
  mutate(year_group_midpoint = create_year_group(year)) %>%
  group_by(year_group_midpoint, rcp) %>%
  summarise(P_summer = mean (P_summer, na.rm = TRUE), #average summer precipitation
            P_winter = mean (P_winter, na.rm = TRUE), #average winter precipitation
            Tmean_summer = mean (Tmean_summer, na.rm = TRUE), #avg summer temps
            Tmean_winter = mean (Tmean_winter, na.rm = TRUE) #avg winter temps
  )

#now we add annual average temp and annual precipitation back in.
THR_avg_weather$Tmean <- (THR_avg_weather$Tmean_summer + THR_avg_weather$Tmean_winter) /2
THR_avg_weather$P <- THR_avg_weather$P_summer + THR_avg_weather$P_winter

#split via RCP:
THR_Weather_RCPs <- split (THR_avg_weather, THR_avg_weather$rcp)

#plot average summer and winter temps:
ggplot() + 
  geom_line ( data = THR_Weather_RCPs[["rcp2p6"]], aes (x = year_group_midpoint, y = Tmean_summer, color = "Summer 2.6") ) +
  geom_line ( data = THR_Weather_RCPs[["rcp2p6"]], aes (x = year_group_midpoint, y = Tmean_winter, color = "Winter 2.6") ) +
  
  geom_line ( data = THR_Weather_RCPs[["rcp6p0"]], aes (x = year_group_midpoint, y = Tmean_summer, color = "Summer 6.0") ) +
  geom_line ( data = THR_Weather_RCPs[["rcp6p0"]], aes (x = year_group_midpoint, y = Tmean_winter, color = "Winter 6.0") ) +
  
  geom_line ( data = THR_Weather_RCPs[["rcp8p5"]], aes (x = year_group_midpoint, y = Tmean_summer, color = "Summer 8.5") ) +
  geom_line ( data = THR_Weather_RCPs[["rcp8p5"]], aes (x = year_group_midpoint, y = Tmean_winter, color = "Winter 8.5") ) +
  
  labs (x = "year", y = "average temperature (°C)", title = "Changes in summer and winter temperatures (°C)") +
  scale_color_manual (values = c("Summer 2.6" = "green", "Winter 2.6"="lightgreen", 
                                 "Summer 6.0"="orange","Winter 6.0"="yellow", 
                                 "Summer 8.5"="red", "Winter 8.5"= "pink"))

#this gives me the result I wanted.

#plots used in the study:
#plot the rate of evapotranspiration for a crop over the entire modelled period:
ggplot() + 
  geom_line ( data = avgTS_5_crops[["onions"]], aes (x = year_group_midpoint, y = ETc_2p6, color = "RCP 2.6") ) +
  geom_line ( data = avgTS_5_crops[["onions"]], aes (x = year_group_midpoint, y = ETc_6p0, color = "RCP 6.0") ) +
  geom_line ( data = avgTS_5_crops[["onions"]], aes (x = year_group_midpoint, y = ETc_8p5, color = "RCP 8.5") ) +
  labs (x = "Year", y = "Annual Evapotranspiration (mm)") +
  theme(panel.background = element_rect(fill = "darkgrey"))+
  scale_color_manual (values = c("green", "orange", "red"))

#yield over entire period
ggplot() + 
  geom_line ( data = avgTS_5_crops[["onions"]], aes (x = year_group_midpoint, y = Y_2p6, color = "RCP 2.6") ) +
  geom_line ( data = avgTS_5_crops[["onions"]], aes (x = year_group_midpoint, y = Y_6p0, color = "RCP 6.0") ) +
  geom_line ( data = avgTS_5_crops[["onions"]], aes (x = year_group_midpoint, y = Y_8p5, color = "RCP 8.5") ) +
  geom_smooth(method = "loess", color = "red", se = FALSE) +  # Smoothed line
  labs (x = "Year", y = "Annual Yield (Mg/ha)") +
  scale_color_manual (values = c("green", "orange", "red"))

#plot changing summer and winter temperatures over the entire period:
ggplot() + 
  geom_line ( data = THR_Weather_RCPs[["rcp2p6"]], aes (x = year_group_midpoint, y = Tmean_summer, color = "Summer 2.6") ) +
  geom_line ( data = THR_Weather_RCPs[["rcp2p6"]], aes (x = year_group_midpoint, y = Tmean_winter, color = "Winter 2.6") ) +
  geom_line ( data = THR_Weather_RCPs[["rcp6p0"]], aes (x = year_group_midpoint, y = Tmean_summer, color = "Summer 6.0") ) +
...
#plot changing annual temperatures:
ggplot() + 
  geom_line ( data = THR_Weather_RCPs[["rcp2p6"]], aes (x = year_group_midpoint, y = Tmean, color = "RCP 2.6") ) +
  geom_line ( data = THR_Weather_RCPs[["rcp6p0"]], aes (x = year_group_midpoint, y = Tmean, color = "RCP 6.0") ) +
  geom_line ( data = THR_Weather_RCPs[["rcp8p5"]], aes (x = year_group_midpoint, y = Tmean, color = "RCP 8.5") ) +
  labs (x = "Year", y = "Average Temperature (°C)") +
  theme(panel.background = element_rect(fill = "darkgrey"))+
  scale_color_manual (values = c("RCP 2.6" = "green",
                                 "RCP 6.0"="orange", 
                                 "RCP 8.5"="red"))

#changing annual rainfall:
ggplot() + 
  geom_line ( data = THR_Weather_RCPs[["rcp2p6"]], aes (x = year_group_midpoint, y = P, color = "RCP 2.6") ) +
  geom_line ( data = THR_Weather_RCPs[["rcp6p0"]], aes (x = year_group_midpoint, y = P, color = "RCP 6.0") ) +
  geom_line ( data = THR_Weather_RCPs[["rcp8p5"]], aes (x = year_group_midpoint, y = P, color = "RCP 8.5") ) +
  labs (x = "Year", y = "Annual Precipitation (mm/ha)") +
  scale_color_manual (values = c("RCP 2.6" = "green",
                                 "RCP 6.0"="orange", 
                                 "RCP 8.5"="red"))
